...
class TransactionResourceIT {
    ...
    public static Transaction createEntity(EntityManager em) {
        Transaction transaction = new Transaction().amount(DEFAULT_AMOUNT).createdOn(DEFAULT_CREATED_ON).type(DEFAULT_TYPE);
        // Add required entity
        Investor investor;
        if (TestUtil.findAll(em, Investor.class).isEmpty()) {
            investor = InvestorResourceIT.createEntity(em);
            em.persist(investor);
            em.flush();
        } else {
            investor = TestUtil.findAll(em, Investor.class).get(0);
        }
        transaction.setUser(investor);
        return transaction;
    }
    ...
    public static Transaction createUpdatedEntity(EntityManager em) {
        Transaction transaction = new Transaction().amount(UPDATED_AMOUNT).createdOn(UPDATED_CREATED_ON).type(UPDATED_TYPE);
        // Add required entity
        Investor investor;
        if (TestUtil.findAll(em, Investor.class).isEmpty()) {
            investor = InvestorResourceIT.createUpdatedEntity(em);
            em.persist(investor);
            em.flush();
        } else {
            investor = TestUtil.findAll(em, Investor.class).get(0);
    ...
    void fullUpdateTransactionWithPatch() throws Exception {
        ...
        partialUpdatedTransaction.amount(UPDATED_AMOUNT).createdOn(UPDATED_CREATED_ON).type(UPDATED_TYPE);
